...
<div class="flex flex-wrap pt-20 px-4 sm:px-6 lg:px-8">
    <div class="w-full xl:w-2/3">
        ...
        <div class="flex justify-between items-center mt-3">
            ...
            <div>
                @livewire('components.like-dislike-button', ['video_id' => $video->id])
            </div>
        </div>
        <div class="border-t border-gray-100 my-3"></div>
        <div class="flex justify-between items-center">
            <div>
                @livewire('components.channel-profile', ['channel_id' => $video->channel->id])
            </div>
            <div>
                @guest
                ...
                @auth
                    @if(auth()->user()->id == $video->channel->owner->id)
                        @livewire('components.channel-owner-button', ['channel_id' => $video->channel->id])
                    @else
                        @livewire('components.subscribe-button', ['channel_id' => $video->channel->id])
                    @endif
                @endauth
            </div>
        </div>
        ...
        @if($video->settings()->get('allow_comments', true))
            @livewire('components.comment', ['video_id' => $video->id, 'commenter' => auth()->user()])
        @else
