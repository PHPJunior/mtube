...
<x-livewire-tables::table.cell class="flex justify-end">
    ...
    <a wire:click="$emit('openModal', 'channel.modal.edit-channel', {{ json_encode(["channel_id" => $row->id]) }})" class="cursor-pointer text-blue-600 font-medium hover:text-blue-900 mr-2">Edit</a>
    <a wire:click="$emit('openModal', 'channel.modal.delete-channel', {{ json_encode(["channel_id" => $row->id]) }})" class="cursor-pointer text-red-600 font-medium hover:text-red-900">Delete</a>
</x-livewire-tables::table.cell>
