<div class="flex">
    ...
    <div class="flex-1 border rounded-lg px-2 py-2 sm:px-4 sm:py-4 leading-relaxed">
        <div class="flex justify-between items-center">
            ...
            @auth
                @if(($comment->commenter->id == auth()->user()->id && $comment->commenter->email) || $owner)
                    <div class="flex justify-between items-center">
                        <a wire:click="$emit('openModal','frontend.video.modal.edit-comment', {{ json_encode(["comment_id" => $comment->id]) }})" class="cursor-pointer text-blue-600 text-sm md:text-md font-medium hover:text-blue-900 mr-2">Edit</a>
                        <a wire:click="$emit('openModal','frontend.video.modal.delete-comment', {{ json_encode(["comment_id" => $comment->id]) }})" class="cursor-pointer text-red-600 text-sm md:text-md font-medium hover:text-red-900">Delete</a>
                    </div>
                @endif
            @endauth
        </div>
        ...
        <div class="flex justify-between @if($comment->children()->count()) my-5 @else mt-5 @endif">
            ...
            @auth
            <h4 wire:click="$emit('openModal','frontend.video.modal.reply-comment', {{ json_encode(["comment_id" => $comment->id, 'commenter' => $commenter, 'owner' => $owner]) }})" class="uppercase tracking-wide text-indigo-500 font-bold text-xs cursor-pointer">Reply</h4>
            @endauth
        </div>
        @if($comment->children()->count())
            <div class="space-y-4">
                @foreach($comment->children()->get() as $cc)
                    @include('livewire.frontend.components._partials.parent-comment', ['comment' => $cc, 'child' => true])
                @endforeach
            </div>
        @endif
    </div>
