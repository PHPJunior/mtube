...
{{--    <a wire:click="$emit('openModal', 'backend.admin.modal.delete-admin', {{ json_encode(["admin_id" => $row->id]) }})" class="cursor-pointer text-red-600 font-medium hover:text-red-900">Delete</a>--}}
    @if($row->isBanned())
        <a wire:click="$emit('openModal', 'backend.channel.modal.un-ban-user', {{ json_encode(["user_id" => $row->id]) }})" class="cursor-pointer font-medium mr-2">UnBan</a>
    @else
        <a wire:click="$emit('openModal', 'backend.channel.modal.ban-user', {{ json_encode(["user_id" => $row->id]) }})" class="cursor-pointer text-red-600 font-medium hover:text-red-900">Ban</a>
    @endif
</x-livewire-tables::table.cell>
