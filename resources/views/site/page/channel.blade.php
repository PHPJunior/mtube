        ...
        <div class="pt-16 px-4 sm:px-6 lg:px-8">
            <div class="card border w-full relative flex flex-col mx-auto mb-5">
                ...
                <div class="flex p-1 md:p-6 border-t justify-between items-center">
                    <div>
                        @livewire('components.channel-profile', ['channel_id' => $channel->id])
                    </div>
                    <div>
                        @guest
                        ...
                        @auth
                            @if(auth()->user()->id == $channel->owner->id)
                                @livewire('components.channel-owner-button', ['channel_id' => $channel->id])
                            @else
                                @livewire('components.subscribe-button', ['channel_id' => $channel->id])
                            @endif
                        @endauth
                    </div>
                </div>
            </div>
            @livewire('video.channel-video', ['channel_id' => $channel->id])
        </div>
    </div>
@endsection
